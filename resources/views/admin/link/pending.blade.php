...
@section('content')
    ...
    <div class="x-body">
        ...
        <xblock>
            <button class="layui-btn" onclick="x_admin_show('添加分类','',600,500)">
        ...
        <table class="layui-table layui-form">
            ...
            @foreach($links as $item)
                <tr>
                    ...
                    <td>@if($item->status == 0) 待审核 @elseif($item->status == 1) 正常 @elseif($item->status == 2) 无法访问 @endif</td>
                    <td>@if(isset($item->created_at)){{ $item->created_at->format('Y-m-d H:i:s') }} @endif</td>
                    <td class="td-manage">
                        <button class="layui-btn layui-btn-normal layui-btn-xs" data-id="{{ $item->id }}">激活</button>

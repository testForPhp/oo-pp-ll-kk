...
@section('content')
    ...
    <div class="x-body">
        ...
        <xblock>
            <button class="layui-btn" onclick="x_admin_show('添加链接','/fileStore/link/{{ $sort->id }}/create',600,500)">
        ...
        <table class="layui-table layui-form">
            ...
            @foreach($links as $item)
                <tr>
                    ...
                    <td>@if($item->status == 1)显示 @else 不显示 @endif</td>
                    <td>@if(isset($item->created_at)){{ $item->created_at->format('Y-m-d H:i:s') }} @endif</td>
                    <td class="td-manage">
                        <button class="layui-btn layui-btn-normal layui-btn-xs" data-id="{{ $item->id }}">冻结</button>
